...
async def mark_user_online(user_id: int, company_id: int) -> None:
    redis = get_redis()
    await redis.sadd(f"online:company:{company_id}", str(user_id))


async def mark_user_offline(user_id: int, company_id: int) -> None:
    redis = get_redis()
    await redis.srem(f"online:company:{company_id}", str(user_id))


async def is_user_online(user_id: int, company_id: int) -> bool:
    """Check if user is online within their company or as a global super_admin."""
    redis = get_redis()
    # Check company-scoped key first
